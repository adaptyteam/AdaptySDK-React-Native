    ...
    try {
      ...
      switch (method) {
        ...
        case 'get_paywall_products':
          ...
          const placementId: string = paywall?.placement?.developer_id || 'default';
          const variationId: string = paywall?.variation_id || 'mock_variation_id';
        ...
        case 'adapty_ui_activate':
          console.info(
            `[Adapty Mock] Called ${method}. This is a mock, so no UI is shown.`,
          );
          result = undefined; // void
          break;
        case 'adapty_ui_show_dialog':
          console.info(
            `[Adapty Mock] Called ${method}. This is a mock, so no dialog is shown. Returning primary action.`,
          );
          result = 'primary';
          break;
